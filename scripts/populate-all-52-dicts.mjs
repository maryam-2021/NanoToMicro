// scripts/populate-all-52-dicts.mjs
import fs from 'fs';
...
const enFile = fs.readFileSync(path.join(dictDir, 'en.ts'), 'utf8');

// Comprehensive scientific terminology database for all 52 languages
const DATA = {
  es: {
    units: {
    ...
    convertPage: {
      ...
      introPattern: 'Convierte {from} ({fromSymbol}) a {to} ({toSymbol}) al instante con fórmulas científicas exactas, tablas de conversión y explicaciones paso a paso.',
  ...
  fr: {
    ...
    convertPage: {
      ...
      introPattern: 'Convertissez instantanément {from} ({fromSymbol}) en {to} ({toSymbol}) avec des formules scientifiques exactes, des tableaux de conversion et des explications métriques.',
...
DATA['pt-BR'] = {
  units: { ...DATA.pt.units },
  convertPage: { ...DATA.pt.convertPage, shortcutText: 'Atalho: mova a vírgula três casas decimais para a esquerda.' },
  siPrefixPage: { ...DATA.pt.siPrefixPage },
  chartsPage: { ...DATA.pt.chartsPage },
  errorPage: { ...DATA.pt.errorPage }
};
...
console.log('Tier 1 and Asian core ready.');
