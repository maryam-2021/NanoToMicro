...
export function ArticleLayout({ meta, ctas, children }: Props) {
  ...
  const canonical = `https://www.nanotomicro.com/guides/${meta.slug}/`;
    ...
    jsonLd: [
      {
        ...
        author: { '@type': 'Organization', name: 'NanoToMicro', url: 'https://www.nanotomicro.com/' },
        publisher: { '@type': 'Organization', name: 'NanoToMicro', url: 'https://www.nanotomicro.com/' },
    ...
    <>
      <section className="border-b border-border/60 bg-grid">
        <div className="mx-auto w-full max-w-3xl px-4 pb-8 pt-10 sm:px-6">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-muted-foreground">
            <span className="inline-flex items-center gap-1.5 rounded-full border border-primary/30 bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">{meta.category}</span>
      ...
      <article className="mx-auto w-full max-w-3xl px-4 py-10 sm:px-6">
        ...
        <div className="mt-12 rounded-2xl border border-primary/30 bg-primary/5 p-6">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Free, instant and exact — no sign-up required.</p>
          <div className="mt-4 flex flex-wrap gap-2.5">
            {ctas.map((c) => <Link key={c.to} to={c.to} className="inline-flex items-center gap-1.5 rounded-full bg-primary px-4 py-2 text-sm font-bold text-primary-foreground transition-transform hover:scale-[1.03]">{c.label} <ArrowRight className="h-3.5 w-3.5" /></Link>)}
          </div>
        </div>
        ...
        <div className="mt-10">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-3">
            {related.map((a) => <Link key={a.slug} to={`/guides/${a.slug}/`} className="rounded-xl border border-border bg-card p-4 transition-colors hover:border-primary/40"><span className="text-xs font-semibold text-primary">{a.category}</span><p className="mt-1.5 text-sm font-semibold leading-snug">{a.title}</p></Link>)}
          </div>
        </div>
      </article>
    </>
